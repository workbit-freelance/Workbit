...
pkgdatadir = $(datadir)/WORKBIT
pkgincludedir = $(includedir)/WORKBIT
pkglibdir = $(libdir)/WORKBIT
pkglibexecdir = $(libexecdir)/WORKBIT
...
CONFIG_HEADER = $(top_builddir)/src/config/WORKBIT-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/WORKBIT-config.h.in \
...
ACLOCAL = ${SHELL} /root/workbit/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/workbit/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/workbit/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/workbit/build-aux/missing automake-1.15
AWK = mawk
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = WORKBIT-cli
BITCOIN_DAEMON_NAME = WORKBITd
BITCOIN_GUI_NAME = WORKBIT-qt
BITCOIN_TX_NAME = WORKBIT-tx
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/workbit/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = WORKBIT
PACKAGE_BUGREPORT = www.WORKBIT.org
PACKAGE_NAME = WORKBIT Core
PACKAGE_STRING = WORKBIT Core 1.5.0
PACKAGE_TARNAME = WORKBIT
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /root/workbit
abs_srcdir = /root/workbit
abs_top_builddir = /root/workbit
abs_top_srcdir = /root/workbit
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/workbit/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_WORKBIT_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_WORKBIT_coverage.info test_WORKBIT.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/WORKBIT-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/WORKBIT-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/WORKBIT-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/WORKBIT-config.h
$(top_srcdir)/src/config/WORKBIT-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/WORKBIT-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/WORKBIT-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/WORKBIT-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/WORKBIT-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_WORKBIT.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_WORKBIT -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_WORKBIT_filtered.info: test_WORKBIT.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_WORKBIT_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_WORKBIT_coverage.info: baseline_filtered_combined.info test_WORKBIT_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_WORKBIT_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_WORKBIT_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_WORKBIT_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_WORKBIT.coverage/.dirstamp:  test_WORKBIT_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_WORKBIT.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_WORKBIT.coverage/ total.coverage/ $(OSX_APP)
