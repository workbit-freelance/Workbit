...
#if defined(NDEBUG)
#error "WORKBIT cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uWRB) are considered zero fee (for relaying and mining)
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
	...
	if (!tx.IsCoinBase() && pindexPrev->nHeight >= 91000) {
        if (pvChecks)
        ...
        if (!tx.IsCoinStake()) {
             if (nValueIn < tx.GetValueOut() && pindexPrev->nHeight >= 91000)
            ...
            if (nTxFee < 0 && pindexPrev->nHeight >= 91000)
                return state.DoS(100, error("CheckInputs() : %s nTxFee < 0", tx.GetHash().ToString()),
                    REJECT_INVALID, "bad-txns-fee-negative");
            nFees += nTxFee;
            if (!MoneyRange(nFees) && pindexPrev->nHeight >= 91000)
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // WORKBIT: We use certain sporks during IBD, so check to see if they are
