...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("WORKBITMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("WORKBITMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("WORKBITMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("WORKBIT-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running WORKBITMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
